<script>
    // Diese Unterseite ist die Hauptseite der WildMap-App und bietet eine interaktive Weltkarte, auf der die Kinder die Kontinente erkunden und Tiere entdecken können
    // Hier sind die meisten Funktionen hinterlegt, die für die Interaktion mit der Weltkarte notwendig sind bspw. Zoomen, Kontinente auswählen, Tiere anzeigen, Lesezeichen setzen, Informationen zu den Kontinenten erhalten
    import { onMount } from "svelte";
    import { animal } from "../scripts/animaldata.js";
    ...
    let wikiDataCache = {};
    let countriesData = [];
    let continentFacts = null;

    // Im Prozess wurde eine zweite API eingebunden, um Informationen zu den Kontinenten zu erhalten
    const continentMapping = {
        Afrika: { continent: "Africa" },
        Europa: { continent: "Europe" },
        Asien: { continent: "Asia" },
        Australien: { continent: "Oceania" },
        Antarktis: { continent: "Antarctic" },
        Nordamerika: { continent: "Americas", subregion: "Northern America" },
        Südamerika: { continent: "Americas", subregion: "South America" },
    };
    ...
    async function fetchExcerpt(animalName) {
        try {
            ...
            console.error("fetchExcerpt-Fehler:", err);
            return {
                extract: "",
                image: "",
            };
    ...
    async function loadWikipediaDataForContinent(continent) {
        const animals = getAnimalsForContinent(continent);
        for (const a of animals) {
            if (!wikiDataCache[a.name]) {
                const data = await fetchExcerpt(a.name);
                wikiDataCache[a.name] = data;
            }
        }
    }

    const zoomToContinent = (element) => {
        const bbox = element.getBBox();
        const svgBbox = svgElement.viewBox.baseVal;

        const scale =
            Math.min(svgBbox.width / bbox.width, svgBbox.height / bbox.height) *
            0.8;

        const translateX =
            svgBbox.x + svgBbox.width - (bbox.x + bbox.width / 2) * scale;
        const translateY =
            svgBbox.y + svgBbox.height - (bbox.y + bbox.height / 2) * scale;
    ...
    const handleContinentClick = async (event) => {
        ...
        if (currentZoom === target) {
            ...
            zoomToContinent(target);
            applyOpacity(target.id);
            currentZoom = target;
            selectedContinent = target.id;
            showAnimalList = true;
            showInfo = false;

            await loadWikipediaDataForContinent(selectedContinent);
        }
    };

    const applyOpacity = (activeId) => {
        const continents = svgElement.querySelectorAll(".continent");
        continents.forEach((continent) => {
            continent.style.opacity = continent.id === activeId ? "1" : "0.3";
        });
    };
    ...
    async function toggleInfo() {
        showInfo = !showInfo;
        if (showInfo && countriesData.length === 0) {
            try {
                const response = await fetch(
    ...
    function getAnimalsForContinent(continent) {
        return animal.filter((animal) => animal.locations?.includes(continent));
    }
    ...
    $: if (selectedContinent && countriesData.length > 0) {
        const mapping = continentMapping[selectedContinent];
        let filtered;
        if (
            selectedContinent === "Nordamerika" ||
            selectedContinent === "Südamerika"
        ) {
            ...
            filtered = countriesData.filter(
                (country) =>
                    country.continents &&
                    country.continents.includes(mapping.continent),
            );
            if (mapping.subregion) {
                const filteredBySubregion = filtered.filter(
                    (country) =>
                        country.subregion &&
                        country.subregion
                            .toLowerCase()
                            .includes(mapping.subregion.toLowerCase()),
                );
                if (filteredBySubregion.length > 0) {
                    filtered = filteredBySubregion;
                }
            }
        }
        ...
        continentFacts = { numCountries, totalPopulation, totalArea };
    } else {
        continentFacts = null;
    }
</script>
...
<div class="animal-list" class:visible={showAnimalList}>
    ...
    {#if selectedContinent}
                    ...
                    <li class="animal-card">
                        ...
                        >
                            <div class="card-image">
                                <img
                                    src={wikiDataCache[a.name]?.image
                                        ? wikiDataCache[a.name].image
        ...
        {:else}
            <p>Keine Tiere für diesen Kontinent hinterlegt.</p>
        {/if}
    {/if}
</div>

<!-- Ganz zuletzt wurde noch der Info-Button hinzugefügt, der nur angezeigt wird, wenn man auf einen Kontinent klickt -->
{#if selectedContinent}
    <button class="info-btn" class:active={showInfo} on:click={toggleInfo}>
...
{#if showInfo}
    <div class="info-popup">
        <div class="popup-content">
            {#if continentFacts}
                <p>Fakten zu {selectedContinent}:</p>
                <ul>
                    <li>Anzahl der Länder: {continentFacts.numCountries}</li>
                    <li>
                        Gesamtbevölkerung: {continentFacts.totalPopulation.toLocaleString()}
                    </li>
                    <li>
                        Gesamtfläche: {continentFacts.totalArea.toLocaleString()}
                        km²
                    </li>
                </ul>
            {:else}
                <p>Lade Fakten…</p>
            {/if}
            <button class="popup-close" on:click={toggleInfo}>X</button>
        </div>
    </div>
{/if}
...
<style>
    ...
    @media (max-width: 425px) {
        ...
        /* Hier wird das Info-Pop-up über der Liste zentriert angezeigt */
        .info-popup {
            position: absolute; /* relative zum nächstgelegenen positionierten Container (z. B. der Haupt-Container oder animal-list) */
